...
public class PyramidalBdvDataset extends AbstractContextual implements Pyramidal {
	...
	List<SourceAndConverter<T>> initSourceAndConverters(final PyramidContents<T, V> contents)
	{
		...
		Arrays.setAll(levelToChannels, l ->
				splitInputStackIntoSourceStacks(numChannels, channelAxisIndex, zAxisPresent, timeAxisPresent,
						contents.cachedCellImgs[l]));

		final RandomAccessibleInterval<V>[][] levelToVolatileChannels = new RandomAccessibleInterval[nLevels][];
		Arrays.setAll(levelToVolatileChannels, l ->
				splitInputStackIntoSourceStacks(numChannels, channelAxisIndex, zAxisPresent, timeAxisPresent,
						contents.volatileImgs[l]));
	...
	private static <T extends NativeType<T> & RealType<T>, V extends Volatile<T> & NativeType<V> & RealType<V>>
	SourceAndConverter<T> createSourceAndConverter(
	...
	private static <T> RandomAccessibleInterval<T>[] splitInputStackIntoSourceStacks(
			final int numChannels,
			final int channelAxisIndex,
			final boolean zAxisPresent,
			final boolean timeAxisPresent,
